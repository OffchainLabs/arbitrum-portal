 ...
 * @param params.url - Legacy param. The URL of the web app to start (optional). By default it's `/bridge` and we only use `/bridge` in the codebase now. TODO: remove this.
...
export function login({
  ...
  const network = networkType === 'parentChain' ? getL1NetworkConfig() : getL2NetworkConfig();
...
export const selectTransactionsPanelTab = (tab: 'pending' | 'settled') => {
  ...
  return cy.get('@tab').should('have.attr', 'data-headlessui-state').and('contain', 'selected');
};
...
export const searchAndSelectToken = ({
  ...
  cy.findByRole('button', { name: 'Add New Token' }).should('be.visible').click();
...
export function typeAmount(amount: string | number): Cypress.Chainable<JQuery<HTMLElement>> {
  return cy.findAmountInput().scrollIntoView().type(String(amount), { delay: 0 });
}

export function typeAmount2(amount: string | number): Cypress.Chainable<JQuery<HTMLElement>> {
  return cy.findAmount2Input().scrollIntoView().type(String(amount), { delay: 0 });
}

export function findSourceChainButton(chain: string): Cypress.Chainable<JQuery<HTMLElement>> {
  return cy.findByLabelText(`From: ${chain}`).should('be.visible');
}

export function findDestinationChainButton(chain: string): Cypress.Chainable<JQuery<HTMLElement>> {
...
export function findSelectTokenButton(text: string): Cypress.Chainable<JQuery<HTMLElement>> {
  return cy
