...
describe('Batch Deposit', () => {
  let parentErc20Balance: string;
  let childNativeTokenBalance: string;
  let childErc20Balance: string;
  ...
  beforeEach(() => {
    ...
    getInitialETHBalance(Cypress.env('ARB_RPC_URL'), Cypress.env('ADDRESS')).then(
      (val) => (childNativeTokenBalance = formatAmount(val)),
    );
  ...
  it('should deposit erc-20 and native currency to the same address', () => {
    ...
    context('native currency balance on child chain should not exist', () => {
      cy.findByLabelText(`${nativeTokenSymbol} balance amount on childChain`).should('not.exist');
    });
    ...
    context('should click add native currency button', () => {
      cy.findByLabelText('Add native currency button').should('be.visible').click();
    });
    ...
    context('funds should reach destination account successfully', () => {
      ...
      cy.findByLabelText(`${ERC20TokenSymbol} balance amount on childChain`).should(($el) => {
        const currentBalance = parseFloat($el.text());
        expect(currentBalance).to.be.gt(Number(childErc20Balance));
      });

      cy.findByLabelText(`${nativeTokenSymbol} balance amount on childChain`).should(($el) => {
        const currentBalance = parseFloat($el.text());
      ...
      cy.findByLabelText(`${ERC20TokenSymbol} balance amount on parentChain`).should(($el) => {
        const currentBalance = parseFloat($el.text());
  ...
  it('should deposit erc-20 and native currency to a different address', () => {
    ...
    context('should deposit successfully', () => {
      ...
      cy.findTransactionDetailsCustomDestinationAddress(Cypress.env('CUSTOM_DESTINATION_ADDRESS'));
