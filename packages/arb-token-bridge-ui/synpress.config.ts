...
const tests = process.env.TEST_FILE ? [process.env.TEST_FILE] : specFiles.map((file) => file.file);
...
export default defineConfig({
  ...
  e2e: {
    async setupNodeEvents(on, config) {
      ...
      config.env.CUSTOM_DESTINATION_ADDRESS = await getCustomDestinationAddress();
