...
function getDialogSourceAndDestinationChains({
  ...
  const { isArbitrumOne: isDestinationChainArbOne } = isNetwork(destinationChainId);
...
export function OneNovaTransferDialog(props: UseDialogProps) {
  ...
  const { selectedSourceChainId, selectedDestinationChainId } = getDialogSourceAndDestinationChains(
    {
  ...
  const sourceNetworkSlug = selectedSourceChainId === ChainId.ArbitrumOne ? 'arbitrum' : 'nova';
          ...
          <Tab.Panel className="flex flex-col space-y-4 py-4">
            <div className="flex flex-col space-y-3">
              <p className="font-light">
                Use a third party bridge to transfer funds between Arbitrum One and Arbitrum Nova.
              </p>
            </div>
