...
vi.mock('use-query-params', async () => {
  ...
  return {
    ...actual,
    useQueryParams: vi.fn(() => [mocks.mockQueryParams, mocks.mockSetQueryParams]),
  };
});

export function setupWrapper(query: Partial<DecodedValueMap<QueryParamConfigMap>>) {
  mocks.mockQueryParams = Object.fromEntries(new URLSearchParams(query as Record<string, string>));
  ...
  mocks.mockSetQueryParams.mockImplementation((updates: SetQueryParamsParameters) => {
