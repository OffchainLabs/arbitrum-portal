...
import { Step, TransactionFailedOnNetwork } from './TransactionsTableDetailsSteps';
import { TransactionsTableRowAction } from './TransactionsTableRowAction';

const TeleportMiddleStepFailureExplanationNote = ({ tx }: { tx: TeleporterMergedTransaction }) => {
  return (
    <div className="mt-2">
      <div
        className={twMerge(
      ...
      >
        {!l1L2RetryableRequiresRedeem(tx) ? (
          <CheckCircleIcon height={15} className="h-[15px] w-[15px] shrink-0 text-green-400" />
        ) : (
...
export const TransactionsTableDetailsTeleporterSteps = ({
  ...
  const isFirstRetryableLegResolved = isFirstRetryableLegSucceeded || isFirstRetryableLegFailed;
  ...
  return (
    <>
      ...
      <Step
        ...
        text={`Wait ~${minutesToHumanReadableTime(firstRetryableWaitingDuration)}`}
        endItem={
          !isFirstRetryableLegResolved && (
            <TransferCountdown tx={tx} firstLegOnly={true} textAfterTime="remaining" />
          )
        }
      />
      ...
      <Step
        ...
        text={`Wait ~${minutesToHumanReadableTime(secondRetryableWaitingDuration)}`}
      />
    </>
  );
};
  ...
  if (isFirstRetryableLegFailed) {
    ...
    return (
      <div>
        ...
        {l2ForwarderRequiresRedeem && <TeleportMiddleStepFailureExplanationNote tx={tx} />}
      </div>
    );
  }
