...
function transformIndexerTransfer(params: {
  ...
  const { tx } = params;
  const tokenDetails = 'tokenDetails' in params ? params.tokenDetails : undefined;
  ...
  return {
    ...
    value: utils.formatUnits(tx.amount, tokenDetails?.decimals ?? ether.decimals),
...
export const useIndexerHistory = (
  address?: Address,
): Omit<UseTransactionHistoryResult, 'addPendingTransaction' | 'updatePendingTransaction'> => {
  ...
  const { pendingTransfers, completedTransfers, isLoading, error } = useArbitrumIndexer(
    isIndexerEnabled ? _address : '',
  );
  ...
  const { data: transactions = [] } = useSWR(
    ...
    ([_indexerTransactions, _tokenDetailsMap]) => {
      return _indexerTransactions.map((tx) => {
        ...
        if (!tokenDetails) {
          throw new Error('Failed to fetch token data for ERC-20 indexer transfer.');
        }
