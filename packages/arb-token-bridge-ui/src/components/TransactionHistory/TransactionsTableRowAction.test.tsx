...
describe('TransactionsTableRowAction', () => {
  ...
  it('keeps a settled route settled while execution resumes', async () => {
    ...
    await waitFor(() => {
      expect(mocks.updateLifiTransactionInCache).toHaveBeenCalledWith(settledTransaction, {
        lifiRoute: updatedRoute,
      });
    });
  });
  ...
  it('clears a stale failure status when the resumed route completes', async () => {
    ...
    await waitFor(() => {
      expect(mocks.updateLifiTransactionInCache).toHaveBeenLastCalledWith(failedTransaction, {
