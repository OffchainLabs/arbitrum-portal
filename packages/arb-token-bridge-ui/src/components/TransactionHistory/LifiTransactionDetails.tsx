...
function getLifiTokenFlowItems(tx: LifiMergedTransaction): LifiTokenFlowItem[] {
  ...
  const historySteps = tx.lifiRouteSteps?.flatMap((step, stepIndex) =>
    step.display
      ? [
          {
            id: step.id || String(stepIndex),
            ...step.display,
          },
        ]
      : [],
  );
  const routeSteps = tx.lifiRoute?.steps.map((step, stepIndex) => {
    const toolDetails =
