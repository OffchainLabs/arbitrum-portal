import { ArrowRightIcon, ArrowSmallLeftIcon, XMarkIcon } from '@heroicons/react/24/outline';
...
function usePanel(panel: Panel) {
  ...
  const setPanel = useCallback(() => setCurrentPanel(panel), [panel, setCurrentPanel]);
...
function SearchPanel({ children }: { children: React.ReactNode }) {
  ...
  return <SearchPanelContext.Provider value={provided}>{children}</SearchPanelContext.Provider>;
}
...
function MainPageCTA({
  ...
  return (
    <button
      className={twMerge('arb-hover text-gray flex items-center gap-2 text-sm', className)}
...
function SearchPanelSecondaryPage({ children, ...props }: React.HTMLAttributes<HTMLDivElement>) {
...
function SecondaryPageCTA({
  ...
  return (
    <div className="mt-4 flex justify-start">
      <button
        className={twMerge('arb-hover flex items-center space-x-2 text-sm', className)}
        onClick={onClickHandler}
        {...props}
      >
