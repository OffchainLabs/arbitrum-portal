...
export const SidePanel = ({
  ...
  return (
    <Dialog
      open={open}
      onClose={handleClose}
      className={twMerge('fixed z-40 h-screen max-h-screen', dialogWrapperClassName)}
    >
      <DialogBackdrop
        transition
        className={twMerge(
      ...
      <div className="fixed inset-0 right-0 top-0 flex h-full w-full items-start justify-end">
        ...
        >
          ...
          >
            ...
            <button className="arb-hover" onClick={handleClose} aria-label="Close side panel">
              <XMarkIcon className={twMerge('h-5 w-5 text-white', !heading && 'ml-2')} />
            </button>
          </Dialog.Title>

          {/* Contents of the panel */}
          <div className={twMerge('side-panel-content z-40 h-full px-4 pb-4', heading && 'p-4')}>
            {children}
          </div>
        </Dialog.Panel>
      </div>
