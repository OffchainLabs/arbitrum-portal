...
function isMergedTransaction(tx: Transaction | MergedTransaction): tx is MergedTransaction {
...
export const getDepositStatus = (tx: Transaction | MergedTransaction) => {
  ...
  // A native ETH deposit message has no redeem step, so `FUNDS_DEPOSITED_ON_CHILD` already means
  // the funds arrived. A retryable ticket carrying ETH keeps them in escrow until it is redeemed,
  // including when a router submits the ticket on the sender's behalf back to the sender's own
  // address, so this cannot be inferred from `assetType` and the destination address.
  const isEthDepositMessage = l1ToL2MsgData.isEthDepositMessage === true;
  ...
  switch (l1ToL2MsgData.status) {
    ...
    case ParentToChildMessageStatus.EXPIRED:
      return isEthDepositMessage ? DepositStatus.L2_SUCCESS : DepositStatus.EXPIRED;
    case ParentToChildMessageStatus.FUNDS_DEPOSITED_ON_CHILD: {
      return isEthDepositMessage ? DepositStatus.L2_SUCCESS : DepositStatus.L2_FAILURE;
    }
...
export const transformDeposit = (tx: Transaction): MergedTransaction => {
  return {
    ...
    direction: tx.type,
    status: tx.status,
    createdAt: tx.timestampCreated ? normalizeTimestamp(tx.timestampCreated) : null,
    resolvedAt: tx.timestampResolved ? normalizeTimestamp(tx.timestampResolved) : null,
    txId: tx.txID,
    asset: tx.assetName || '',
...
export const transformWithdrawal = (tx: L2ToL1EventResultPlus): MergedTransaction => {
  ...
  return {
    ...
    direction: 'outbox',
    status:
      tx.nodeBlockDeadline === NodeBlockDeadlineStatusTypes.EXECUTE_CALL_EXCEPTION
        ? 'Failure'
...
export const filterTransactions = (
  ...
  for (const tx of transactions) {
    ...
    const matchesL2 =
      typeof txL2NetworkID === 'undefined' ? matchesL1 : txL2NetworkID === String(l2ChainId);
