...
export async function GET(request: NextRequest): Promise<NextResponse<EthTeleportResponse>> {
  try {
  ...
  `;
    ...
    const transactions: FetchEthTeleportsFromSubgraphResult[] = subgraphResult.data.retryables.map(
