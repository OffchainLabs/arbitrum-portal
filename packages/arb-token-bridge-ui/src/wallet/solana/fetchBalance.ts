...
import type { FetchBalanceInput, SolanaBalanceClient } from '../types';

export const solanaNativeTokenAddress = SOLANA_NATIVE_TOKEN_ADDRESS;
...
export function createSolanaBalanceClient(client: SolanaBalanceRpcClient): SolanaBalanceClient {
  return {
    ecosystem: 'solana',
    async fetchBalance({ chainId, walletAddress, tokenAddresses }: FetchBalanceInput) {
      ...
      const requestedTokenAddresses = new Set(
        tokenAddresses.filter((tokenAddress) => tokenAddress !== solanaNativeTokenAddress),
      );
