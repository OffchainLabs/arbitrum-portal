...
import { MockInstance, afterAll, beforeAll, describe, expect, it, vi } from 'vitest';
...
describe('getSupportedChainIds', () => {
  describe('includeMainnets is true, includeTestnets is unset', () => {
    it('should return a list of chain ids that includes Mainnets', () => {
      expect(getSupportedChainIds({ includeMainnets: true })).toContain(ChainId.Ethereum);
      expect(getSupportedChainIds({ includeMainnets: true })).toContain(ChainId.ArbitrumOne);
      expect(getSupportedChainIds({ includeMainnets: true })).toContain(ChainId.ArbitrumNova);
    });
    it('should return a list of chain ids that does not include Testnets', () => {
      expect(getSupportedChainIds({ includeMainnets: true })).not.toContain(ChainId.Sepolia);
      expect(getSupportedChainIds({ includeMainnets: true })).not.toContain(
        ChainId.ArbitrumSepolia,
      );
      expect(getSupportedChainIds({ includeMainnets: true })).not.toContain(ChainId.Local);
      expect(getSupportedChainIds({ includeMainnets: true })).not.toContain(ChainId.ArbitrumLocal);
    });
  });
  describe('includeMainnets is true, includeTestnets is true', () => {
    it('should return a list of chain ids that includes Mainnets', () => {
      expect(getSupportedChainIds({ includeMainnets: true, includeTestnets: true })).toContain(
        ChainId.Ethereum,
      );
      expect(getSupportedChainIds({ includeMainnets: true, includeTestnets: true })).toContain(
        ChainId.ArbitrumOne,
  ...
  describe('includeMainnets is unset, includeTestnets is true', () => {
    it('should return a list of chain ids that includes Mainnets', () => {
      expect(getSupportedChainIds({ includeTestnets: true })).toContain(ChainId.Ethereum);
      expect(getSupportedChainIds({ includeTestnets: true })).toContain(ChainId.ArbitrumOne);
      expect(getSupportedChainIds({ includeTestnets: true })).toContain(ChainId.ArbitrumNova);
    });
    it('should return a list of chain ids that includes Testnets', () => {
      expect(getSupportedChainIds({ includeTestnets: true })).toContain(ChainId.Sepolia);
      expect(getSupportedChainIds({ includeTestnets: true })).toContain(ChainId.ArbitrumSepolia);
      expect(getSupportedChainIds({ includeTestnets: true })).toContain(ChainId.Local);
      expect(getSupportedChainIds({ includeTestnets: true })).toContain(ChainId.ArbitrumLocal);
    });
  });
  describe('includeMainnets is false, includeTestnets is false', () => {
    it('should return a list of chain ids that includes Mainnets', () => {
      expect(getSupportedChainIds({ includeMainnets: false, includeTestnets: false })).toHaveLength(
        0,
      );
    });
  });
...
describe('getDestinationChainIds', () => {
    ...
    localStorageGetItemMock = vi
      ...
      .mockImplementation((key: string) => {
        if (key === customChainLocalStorageKey) {
          return JSON.stringify([mockedOrbitChain_1, mockedOrbitChain_2, mockedOrbitChain_3]);
        }
        return null;
      });
  ...
  function isAscending(arr: number[]) {
    return arr.every((value, index) => index === 0 || value >= Number(arr[index - 1]));
  }
  ...
  describe('should return for Base', () => {
    ...
    it('a sorted list with lifi destinations', () => {
      const [arbitrumOne, ...orbitChains] = getDestinationChainIds(ChainId.Base, {
        includeLifiEnabledChainPairs: true,
      });
  ...
  describe('when `disableTransfersToNonArbitrumChains` is true', () => {
    it('should exclude non-Arbitrum networks as destination', () => {
      ...
      expect(result).toEqual([3333, ChainId.ApeChain]);

      const result2 = getDestinationChainIds(ChainId.ArbitrumOne, {
        disableTransfersToNonArbitrumChains: true,
        includeLifiEnabledChainPairs: true,
      });
      expect(result2).toEqual([ChainId.RobinhoodChain, 3333, ChainId.ApeChain]);
    });
  });
  ...
  describe('when `disableTransfersToNonArbitrumChains` is false (defaut behavior)', () => {
    it('should include non-Arbitrum networks as destination', () => {
      ...
      expect(result).toEqual([ChainId.Ethereum, 3333, ChainId.ApeChain]);

      const result2 = getDestinationChainIds(ChainId.ArbitrumOne, {
        disableTransfersToNonArbitrumChains: false,
        includeLifiEnabledChainPairs: true,
      });
      expect(result2).toEqual([1, ChainId.RobinhoodChain, 3333, ChainId.ApeChain]);
  ...
  describe('when `includeLifiEnabledChainPairs` is true', () => {
    it('should include all lifi destinations', () => {
      ...
      expect(result).toEqual([ChainId.RobinhoodChain, 3333, ChainId.ApeChain]);
      ...
      expect(result2).toEqual([ChainId.Ethereum, ChainId.RobinhoodChain, 3333, ChainId.ApeChain]);
  ...
  describe('when `includeLifiEnabledChainPairs` is false (defaut behavior)', () => {
    it('should exclude all lifi-only destinations', () => {
      ...
      expect(result2).toEqual([ChainId.Ethereum, 3333, ChainId.ApeChain]);
