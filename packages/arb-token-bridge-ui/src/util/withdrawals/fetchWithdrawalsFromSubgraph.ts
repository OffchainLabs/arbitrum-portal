import { hasBridgeSubgraph } from '../SubgraphUtils';
...
export async function fetchWithdrawalsFromSubgraph({
  ...
  if (!hasBridgeSubgraph(Number(l2ChainId)) && !isChildChainIndexed(Number(l2ChainId))) {
    throw new Error(`Withdrawal history not available for network: ${l2ChainId}`);
  }
