...
describe('fetchTokenWithdrawalsFromSubgraph', () => {
  it('fetches no withdrawals from subgraph pre-nitro', async () => {
    const result = await fetchWithdrawalsFromSubgraph(getQueryCoveringClassicOnlyWithoutResults());

    expect(result).toHaveLength(0);
  });

  it('fetches some withdrawals from subgraph pre-nitro', async () => {
    const result = await fetchWithdrawalsFromSubgraph(getQueryCoveringClassicOnlyWithResults());

    expect(result).toHaveLength(2);
    expect(result).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          l2TxHash: '0x4142fe27aa74108010135eac0c2d10bc518a1c683d7f7d3f90fa00fe383bcb25',
        }),
        expect.objectContaining({
          l2TxHash: '0x98c3c3bf97f177e80ac5a5adb154208e5ad43120455e624fff917a7546273800',
        }),
      ]),
    );
  });
  ...
  it('fetches some withdrawals from subgraph pre-nitro and post-nitro', async () => {
    const result = await fetchWithdrawalsFromSubgraph(getQueryCoveringClassicAndNitroWithResults());
