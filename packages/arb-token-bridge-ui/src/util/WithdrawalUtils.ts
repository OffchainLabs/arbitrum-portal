...
export async function withdrawInitTxEstimateGas({
  ...
  try {
    ...
    const estimatedChildChainGas = BigNumber.from(
      Math.ceil(Number(await childChainProvider.estimateGas(withdrawalRequest.txRequest)) * 1.3),
    );
    ...
    return {
      ...
      estimatedChildChainGas: isToken ? BigNumber.from(1_400_000) : BigNumber.from(800_000),
      isError: true,
    };
  }
}
...
export function getConfirmationTime(chainId: number) {
  ...
  const confirmationTimeInReadableFormat = formatDuration(confirmationTimeInSeconds);
  const confirmationTimeInReadableFormatShort = formatDuration(confirmationTimeInSeconds, true);
