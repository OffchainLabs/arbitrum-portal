...
async function fetchTokenFallbackGasEstimates({
  ...
  const estimatedChildChainSubmissionCost = await inbox.calculateRetryableSubmissionFee(
...
async function allowanceIsInsufficient(params: DepositTokenEstimateGasParams) {
  const { amount, address, parentChainErc20Address, parentChainProvider, childChainProvider } =
    params;
