...
export function getLifiRouteStatusRequest(route: RouteExtended | undefined) {
  const steps = [...(route?.steps ?? []).entries()];
  const candidates = steps.filter(([, step]) => step.execution?.status !== 'DONE');
  for (const [stepIndex, step] of [
    ...candidates,
    ...steps.filter(([, step]) => step.execution?.status === 'DONE').reverse(),
  ]) {
    const transferProcess = step.execution?.process.find(
      (process) =>
        (step.action.fromChainId !== step.action.toChainId
          ? process.type === 'CROSS_CHAIN'
          : LIFI_TRANSFER_PROCESS_TYPES.has(process.type)) && process.status !== 'FAILED',
    );
    const txHash = transferProcess?.txHash;

    if (
      transferProcess &&
      !isPendingLifiProcessId(transferProcess) &&
