...
async function customFeeTokenAllowanceIsInsufficient(params: DepositEthEstimateGasParams) {
  ...
  if (typeof l2Network.nativeToken === 'undefined') {
    throw new Error('[customFeeTokenAllowanceIsInsufficient] expected nativeToken to be defined');
  }
...
export type DepositEthEstimateGasParams = Omit<DepositTxEstimateGasParams, 'erc20L1Address'>;

export async function depositEthEstimateGas(
  params: DepositEthEstimateGasParams,
): Promise<DepositGasEstimates> {
  const { amount, address, parentChainProvider, childChainProvider, destinationAddress } = params;
  ...
  try {
    ...
    return {
      ...
      estimatedChildChainSubmissionCost: depositToRequest.retryableData.maxSubmissionCost,
    };
  } catch (_) {
