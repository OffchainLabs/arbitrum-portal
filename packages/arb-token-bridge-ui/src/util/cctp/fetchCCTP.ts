...
function mapCCTPTransfer<T extends PendingCCTPTransfer | CompletedCCTPTransfer>(
  ...
  cctpTransfer.messageSent.amount = convertStringToUsdcBigNumber(cctpTransfer.messageSent.amount);
  return cctpTransfer;
}
...
async function fetchCCTP({
...
}: FetchParams & { type: 'deposits' | 'withdrawals' }): Promise<Response['data']> {
  ...
  const response = await fetch(`${getAPIBaseUrl()}/api/cctp/${type}?${urlParams}`, {
    method: 'GET',
  ...
  return {
    pending: sanitizedPendingTransfers.map((transfer) => mapCCTPTransfer(transfer)),
    completed: sanitizedCompletedTransfers.map((transfer) => mapCCTPTransfer(transfer)),
  };
}

type FetchCctpResponse = Response['data'];
export async function fetchCCTPDeposits(params: FetchParams): Promise<FetchCctpResponse> {
  return fetchCCTP({ ...params, type: 'deposits' });
}

export async function fetchCCTPWithdrawals(params: FetchParams): Promise<FetchCctpResponse> {
