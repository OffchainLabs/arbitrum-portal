import { ParentToChildMessageStatus, ParentToChildMessageWriter } from '@arbitrum/sdk';
...
import { DepositStatus, MergedTransaction, TeleporterMergedTransaction } from '../state/app/state';
import { L2ToL3MessageData, isTeleportTx } from '../types/Transactions';
...
const redeemTeleporterFirstLeg = async ({
  ...
  if (l2ForwarderRetryableRequiresRedeem(teleportTransfer)) {
    ...
    const l2ForwarderRetryable = await getRetryableTicket({
      parentChainTxHash: tx.txId,
      retryableCreationId: teleportTransfer?.l2ToL3MsgData?.l2ForwarderRetryableTxID,
      parentChainProvider: getProviderForChainId(tx.parentChainId),
      childChainSigner: signer,
    });
    ...
    // update the teleport tx in the UI
    const updatedTeleportTransfer = await getUpdatedTeleportTransfer(teleportTransfer);
...
export function useRedeemTeleporter(
  ...
  const redeem = useCallback(async () => {
    ...
    if (!isTeleportTx(tx)) {
      throw new Error('The transaction being redeemed is not a LayerLeap transaction.');
    }
    ...
    try {
      ...
      return errorToast(`There was an error, here is more information: ${error.message}`);
    } finally {
