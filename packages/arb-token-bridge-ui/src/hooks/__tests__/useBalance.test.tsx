...
const provider = new StaticJsonRpcProvider(process.env.NEXT_PUBLIC_RPC_URL_ETHEREUM, 1);
...
describe.sequential('useBalance', () => {
  ...
  it('getter returns null for undefined walletAddress', async () => {
    ...
    const { result } = renderHook(() => useBalance({ chainId: 1, walletAddress: undefined }), {
      wrapper: Container,
    });
  ...
  describe('ETH Balance', () => {
    it('getter returns ETH balance for valid tuple (walletAddress, chainId)', async () => {
      ...
      const { result } = renderHook(() => useBalance({ chainId: 1, walletAddress }), {
        wrapper: Container,
      });
...
describe.sequential('ERC20 Balance', () => {
  it('getter returns ERC20 balance for valid tuple (walletAddress, chainId)', async () => {
    ...
    const getTokenDataSpy = vi.spyOn(MultiCaller.prototype, 'getTokenData').mockResolvedValueOnce([
      {
