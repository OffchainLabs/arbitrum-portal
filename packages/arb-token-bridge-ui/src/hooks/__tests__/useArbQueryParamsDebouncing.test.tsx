...
describe.sequential('useArbQueryParams debouncing', () => {
  ...
  it('should be batched with debounce: true', async () => {
    ...
    await act(async () => {
      ...
      setQueryParams({ token: '0xaf88d065e77c8cc2239327c5edb3a432268e5831' }, { debounce: true });
  ...
  it('should flush pending updates when receiving a call with debounce: false', async () => {
    ...
    await act(async () => {
      ...
      setQueryParams({ amount: '10.5' }, { debounce: true });
      setQueryParams({ sourceChain: ChainId.ArbitrumOne }, { debounce: true });
      setQueryParams({ destinationChain: ChainId.Ethereum }, { debounce: false });
      setQueryParams({ token: '0xaf88d065e77c8cc2239327c5edb3a432268e5831' }, { debounce: true });
      vi.runOnlyPendingTimers();
    });
  ...
  it('should handle mixed object and function updates correctly', async () => {
    ...
    await act(async () => {
      ...
      setQueryParams({ destinationChain: ChainId.ArbitrumOne }, { debounce: true });
      setQueryParams({ token: '0x833589fcd6edb6e08f4c7c32d4f71b54bda02913' }, { debounce: true });
    ...
    const secondSetQueryParamsResult = secondSetQueryParams(setQueryParamsResult);
