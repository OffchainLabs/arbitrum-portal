...
describe('useBalanceOnSourceChain', () => {
  it('uses the Solana wallet and native sentinel without reading EVM native-currency data', () => {
    ...
    const { result } = renderHook(() => useBalanceOnSourceChain(null));

    expect(result.current).toEqual(BigNumber.from(123));
    expect(useNativeCurrency).toHaveBeenCalledWith({
      provider: getProviderForChainId(ChainId.Solana),
      enabled: false,
    });
