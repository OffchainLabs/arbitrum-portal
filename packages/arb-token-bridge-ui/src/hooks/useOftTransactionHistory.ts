...
const getOftTransactionStatus = (message: LayerZeroMessage) => {
  return (
    LayerZeroMessageStatus[message.status.name as keyof typeof LayerZeroMessageStatus] ?? 'pending'
  );
};
...
function mapLayerZeroMessageToLayerZeroTransaction(
  ...
  return {
    ...
    resolvedAt:
      getOftTransactionStatus(message) === 'pending' ? null : new Date(message.updated).getTime(),
    ...
    asset: 'USDT',
    value: '0',
    tokenAddress: isDeposit ? CommonAddress.Ethereum.USDT : CommonAddress.ArbitrumOne.USDT,
  };
}
...
export async function getUpdatedOftTransfer(
  ...
  try {
    ...
    return {
      ...tx,
      status,
      resolvedAt: status === 'pending' ? null : new Date(message.updated).getTime(),
      destinationTxHash,
    };
  } catch (error) {
