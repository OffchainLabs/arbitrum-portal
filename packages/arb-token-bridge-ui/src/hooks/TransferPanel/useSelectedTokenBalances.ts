  ...
  return useMemo(() => {
    ...
    if (addressesEqual(selectedToken.address, constants.AddressZero)) {
      ...
      if (!sourceNativeCurrency.isCustom) {
        ...
        sourceBalance =
          (selectedToken.l2Address && erc20SourceBalances?.[selectedToken.l2Address]) || null;
      }
      ...
      if (!destinationNativeCurrency.isCustom) {
        ...
        destinationBalance =
          (selectedToken.l2Address && erc20DestinationBalances?.[selectedToken.l2Address]) || null;
      }
    ...
    if (erc20ParentBalances) {
      parentBalance = erc20ParentBalances[selectedToken.address.toLowerCase()] ?? null;
    }
    ...
    if (
      ...
      childBalance = erc20ChildBalances[selectedToken.l2Address.toLowerCase()] ?? constants.Zero;
    }
