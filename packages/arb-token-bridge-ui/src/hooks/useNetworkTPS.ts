...
const fetchNetworkTPS = async (l2ChainId: number) => {
  ...
  const canFetchTPS =
    isNetwork(Number(l2ChainId)).isArbitrumNova || isNetwork(Number(l2ChainId)).isArbitrumOne;
  ...
  const tps = (tpsElement?.text() || '').trim().replace(/[()]/g, '').split(' ')[0];
...
export const useNetworkTPS = () => {
  ...
  return useSWR(['tps', childChain.id], ([, _childChainId]) => fetchNetworkTPS(_childChainId), {
