...
export const useArbTokenBridge = (params: TokenBridgeParams): ArbTokenBridge => {
  ...
  const { l1TokenAddresses, l2TokenAddresses } = useMemo(() => {
    const tokens = Object.values(bridgeTokens ?? {}).filter(
      (token): token is ERC20BridgeToken => !!token,
    );
    const l1Addresses = tokens
      .filter((token) => isTokenAvailableOnChain(token, l1.network.id))
      .map((token) => token.address);
    const l2Addresses = tokens
      .filter((token) => token.l2Address && isTokenAvailableOnChain(token, l2.network.id))
      .map((token) => token.l2Address as string);

    if (isNetwork(l2.network.id).isArbitrumOne) {
      l2Addresses.push(CommonAddress.ArbitrumOne.USDC);
    }
    if (isNetwork(l2.network.id).isArbitrumSepolia) {
      l2Addresses.push(CommonAddress.ArbitrumSepolia.USDC);
    }

    return {
      l1TokenAddresses: Array.from(new Set(l1Addresses)),
      l2TokenAddresses: Array.from(new Set(l2Addresses)),
    };
  }, [bridgeTokens, l1.network.id, l2.network.id]);

  useTokenBalances({
    chainId: l1.network.id,
    walletAddress,
    tokenAddresses: l1TokenAddresses,
  });
  useTokenBalances({
    chainId: l2.network.id,
    walletAddress,
    tokenAddresses: l2TokenAddresses,
  });
  const customDestinationAddress =
    destinationAddress && isAddress(destinationAddress) ? destinationAddress : undefined;
  useTokenBalances({
    chainId: l1.network.id,
    walletAddress: customDestinationAddress,
    tokenAddresses: l1TokenAddresses,
  });
  useTokenBalances({
    chainId: l2.network.id,
