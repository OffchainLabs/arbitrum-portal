...
export type TransferCallbacks = {
  ...
  removeLifiTransactionFromCache: (tx: LifiMergedTransaction) => void;
...
const transferNetworkSwitchers: Partial<
  Record<WalletEcosystem, (chainId: number) => Promise<unknown>>
> = {
  evm: switchEvmTransferNetwork,
};

export async function switchTransferNetwork(chainId: number) {
  const switchNetwork = transferNetworkSwitchers[getWalletEcosystem(chainId)];
  if (!switchNetwork) throw new Error('Network switching is unavailable for this ecosystem.');
  return switchNetwork(chainId);
}

export async function executeTransfer<Ecosystem extends string = WalletEcosystem>(
  snapshot: TransferSubmission,
  callbacks: TransferCallbacks,
  implementations: Partial<Record<Ecosystem, TransferExecutor>> = transferExecutors as Partial<
    Record<Ecosystem, TransferExecutor>
  >,
  ecosystemForChain: (chainId: number) => Ecosystem = getWalletEcosystem as (
    chainId: number,
  ) => Ecosystem,
) {
