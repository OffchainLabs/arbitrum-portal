...
import { getAddressFromSigner, getChainIdFromProvider, percentIncrease } from './utils';
...
export class Erc20WithdrawalStarter extends BridgeTransferStarter {
  ...
  private async getSourceChainGatewayAddress(): Promise<string> {
    const destinationChainErc20Address = await this.getDestinationChainErc20Address();
  ...
  public async approveTokenEstimateGas({ signer, amount }: ApproveTokenProps) {
    ...
    return contract.estimateGas.approve(gatewayAddress, amount ?? constants.MaxUint256, {
      from: address,
    });
  }
  ...
  public async approveToken({ signer, amount }: ApproveTokenProps) {
    ...
    await contract.functions.approve(gatewayAddress, amount ?? constants.MaxUint256);
  }
  ...
  public async transfer({ amount, signer, destinationAddress }: TransferProps) {
    ...
    const erc20Bridger = await Erc20Bridger.fromProvider(this.sourceChainProvider);
