...
vi.mock('../util/LifiTransactionStatus', () => ({
  getExecutedLifiRouteTxHash: (route: RouteExtended) =>
    route.toAmount === '1' ? 'submitted-id' : undefined,
  getPendingLifiRouteBatchIds: () => [],
  rejectPendingLifiRouteRequest: (route: RouteExtended) => route,
}));
...
describe.sequential('LiFi execution lifecycle', () => {
  ...
  it('reports a later failure without attempting another submission', async () => {
    ...
    await vi.waitFor(() =>
      expect(effects.onRouteExecutionError).toHaveBeenCalledWith(error, submitted),
    );
  ...
  it('rejects before submission when the user declines approval', async () => {
    ...
    expect(effects.onRouteExecutionError).toHaveBeenCalledWith(expect.any(Error), undefined);
  });
  ...
  it('resumes the captured route with the same account guard', async () => {
    ...
    const effects = callbacks();
    await expect(resumeLifiRoute(submitted, effects)).resolves.toEqual(submitted);
