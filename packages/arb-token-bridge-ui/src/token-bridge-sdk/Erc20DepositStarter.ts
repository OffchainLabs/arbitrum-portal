import { Erc20Bridger, scaleFrom18DecimalsToNativeTokenDecimals } from '@arbitrum/sdk';
import { ERC20__factory } from '@arbitrum/sdk/dist/lib/abi/factories/ERC20__factory';
import { BigNumber, constants, utils } from 'ethers';

import { depositTokenEstimateGas } from '../util/TokenDepositUtils';
import { fetchErc20Allowance, fetchErc20ParentChainGatewayAddress } from '../util/TokenUtils';
import {
...
export class Erc20DepositStarter extends BridgeTransferStarter {
  ...
  private async getBridger() {
    ...
    this.erc20Bridger = await Erc20Bridger.fromProvider(this.destinationChainProvider);
  ...
  public async requiresNativeCurrencyApproval({
    ...
    const nativeCurrency = ERC20__factory.connect(l2Network.nativeToken, this.sourceChainProvider);

    const customFeeTokenAllowanceForSourceChainGateway = await fetchErc20Allowance({
    ...
    const destinationChainGasPrice = await this.destinationChainProvider.getGasPrice();

    const estimatedDestinationChainGasFeeEth = parseFloat(
      utils.formatEther(
        gasEstimates.estimatedChildChainGas
          .mul(percentIncrease(destinationChainGasPrice, DEFAULT_GAS_PRICE_PERCENT_INCREASE))
          .add(gasEstimates.estimatedChildChainSubmissionCost),
      ),
    );

    const estimatedDestinationChainGasFee = scaleFrom18DecimalsToNativeTokenDecimals({
    ...
    return customFeeTokenAllowanceForSourceChainGateway.lt(
      estimatedDestinationChainGasFee.add(options?.approvalAmountIncrease ?? BigNumber.from(0)),
    );
  }
  ...
  public async approveNativeCurrency({ signer, amount, options }: ApproveNativeCurrencyProps) {
    if (!this.sourceChainErc20Address) {
      throw Error('Erc20 token address not found');
    }
  ...
  public requiresTokenApproval = async ({ amount, owner }: RequiresTokenApprovalProps) => {
    if (!this.sourceChainErc20Address) {
      throw Error('Erc20 token address not found');
    }
  ...
  public async approveTokenEstimateGas({ signer, amount }: ApproveTokenProps) {
    ...
    const contract = ERC20__factory.connect(this.sourceChainErc20Address, this.sourceChainProvider);
  ...
  public async transfer({ amount, signer, destinationAddress, overrides }: TransferProps) {
    if (!this.sourceChainErc20Address) {
      throw Error('Erc20 token address not found');
    }
    ...
    const gasLimit = await this.sourceChainProvider.estimateGas(depositRequest.txRequest);
