...
export class EthDepositStarter extends BridgeTransferStarter {
  ...
  private async getBridger(): Promise<EthBridger> {
    ...
    this.ethBridger = await EthBridger.fromProvider(this.destinationChainProvider);
  ...
  public async transferEstimateGas({ amount, from, destinationAddress }: TransferEstimateGasProps) {
