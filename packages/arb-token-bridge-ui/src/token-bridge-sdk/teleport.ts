...
import { Erc20L1L3Bridger, EthL1L3Bridger, getArbitrumNetwork } from '@arbitrum/sdk';
...
async function tryGetInboxFromRouter(
  ...
  if (!(await addressIsSmartContract(address, chainId))) {
    throw new Error('[tryGetInboxFromRouter]: address passed is not a smart contract');
  }

  if ((await getAccountType({ address, chainId })) === 'delegated-account') {
    throw new Error('[tryGetInboxFromRouter]: address passed is a delegated wallet');
  }
...
async function getChainIdFromInbox(address: string, provider: Provider): Promise<number> {
...
async function getChainIdFromInboxOrRouter(address: string, provider: Provider): Promise<number> {
...
export const getL3ChainIdFromTeleportEvents = async (
  ...
  const l3ChainId = await getChainIdFromInboxOrRouter(l2l3RouterOrInbox, l2Provider);
