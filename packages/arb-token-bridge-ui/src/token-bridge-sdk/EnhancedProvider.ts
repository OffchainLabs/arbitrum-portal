import { Networkish, StaticJsonRpcProvider, TransactionReceipt } from '@ethersproject/providers';
 ...
 */
export const shouldCacheTxReceipt = (chainId: number, txReceipt: TransactionReceipt): boolean => {
...
function getTxReceiptFromCache(storage: Storage, chainId: number, txHash: string) {
...
function addTxReceiptToCache(storage: Storage, chainId: number, txReceipt: TransactionReceipt) {
  const cachedReceipts = JSON.parse(storage.getItem(localStorageKey) || '{}');
