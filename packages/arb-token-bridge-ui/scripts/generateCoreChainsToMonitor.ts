...
async function generateCoreChainsToMonitor() {
  const novaChain = {
    ...
    rpcURL: sanitizeEnvValue(process.env.NOVA_MONITOR_RPC_URL) ?? rpcURLs[ChainId.ArbitrumNova],
  };
  const arbOneChain = {
    ...getArbitrumNetwork(ChainId.ArbitrumOne),
    rpcURL: sanitizeEnvValue(process.env.ARB_ONE_MONITOR_RPC_URL) ?? rpcURLs[ChainId.ArbitrumOne],
  };
