...
export const chainSchema = z
  ...
  .superRefine(async (chain, ctx) => {
    ...
    const parentAddressesToCheck = [
      ...
      (address): address is string => typeof address === 'string' && address !== ZERO_ADDRESS,
    );
...
export const validateIncomingChainData = async (rawData: unknown): Promise<IncomingChainData> => {
  try {
