...
import { mockOrbitChain, mockValidTokenBridge } from './__mocks__/chainDataMocks';
...
describe('Validation Functions', () => {
  describe('isValidAddress', () => {
    ...
    it('should return false for invalid Ethereum addresses', () => {
      ...
      expect(isValidAddress('0x742d35Cc6634C0532925a3b844Bc454e4438f44e0')).toBe(false); // Too long
      expect(isValidAddress('0x742d35Cc6634C0532925a3b844Bc454e4438f44G')).toBe(false); // Invalid character
  ...
  describe('ethBridgeSchema', () => {
    it('should validate correct ethBridge objects', async () => {
      ...
      await expect(ethBridgeSchema.parseAsync(validEthBridge)).resolves.not.toThrow();
    });
    ...
    it('should throw for invalid ethBridge objects', async () => {
      ...
      await expect(ethBridgeSchema.parseAsync(invalidEthBridge)).rejects.toThrow();
    });
  });
  ...
  describe('bridgeUiConfigSchema', () => {
    it('should validate correct bridgeUiConfig objects', () => {
      ...
      expect(() => bridgeUiConfigSchema.parse(validBridgeUiConfig)).not.toThrow();
    });
  ...
  describe('chainSchema', () => {
    it('should validate correct chain objects', async () => {
      await expect(chainSchema.parseAsync(mockOrbitChain)).resolves.not.toThrow();
    }, 1000000);
