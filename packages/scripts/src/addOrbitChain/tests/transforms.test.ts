...
import { fullMockIssue, mockIncomingChainData, mockOrbitChain } from './__mocks__/chainDataMocks';
...
describe('Transforms', () => {
  ...
  describe('updateOrbitChainsFile', () => {
    ...
    it('should update the orbit chains file correctly while preserving order', () => {
      ...
      expect(result.mainnet.map((chain: any) => chain.chainId)).toEqual([2, 4, 1, 5]);
      expect(result.testnet.map((chain: any) => chain.chainId)).toEqual([3]);
      expect(result.mainnet.find((chain: any) => chain.chainId === 5)).toEqual(newChain);
    ...
    it('should add a new testnet chain while preserving order', () => {
      ...
      expect(result.mainnet.map((chain: any) => chain.chainId)).toEqual([2, 4, 1]);
      expect(result.testnet.map((chain: any) => chain.chainId)).toEqual([3, 5]);
      expect(result.testnet.find((chain: any) => chain.chainId === 5)).toEqual(newTestnetChain);
    ...
    it('should handle updating an existing chain while preserving order', () => {
      ...
      expect(result.mainnet.map((chain: any) => chain.chainId)).toEqual([2, 4, 1]);
      expect(result.testnet.map((chain: any) => chain.chainId)).toEqual([3]);
      expect(result.mainnet.find((chain: any) => chain.chainId === 2)).toEqual(updatedChain);
  ...
  describe('resizeImage', () => {
    ...
    const resizedImagePath = path.join(__dirname, '__mocks__', 'resized-test-image.jpg');
    ...
    it('should resize an image to under 100KB while maintaining aspect ratio', async () => {
      ...
      expect(resizedMetadata.height).toBeLessThanOrEqual(originalMetadata.height!);

      const originalAspectRatio = originalMetadata.width! / originalMetadata.height!;
      const resizedAspectRatio = resizedMetadata.width! / resizedMetadata.height!;
  ...
  describe('Image Download and Processing', () => {
    ...
    it('should correctly identify and handle SVG images without extension', async () => {
      ...
      const { buffer, fileExtension } = await fetchAndProcessImage('https://example.com/logo');
...
const saveImageLocally = (imageBuffer: Buffer, fileName: string, fileExtension: string): string => {
  ...
  if (fs.existsSync(fullSavePath)) {
    warning(`${fileName} already exists at '${imageSavePath}'. Overwriting the existing image.`);
  }
