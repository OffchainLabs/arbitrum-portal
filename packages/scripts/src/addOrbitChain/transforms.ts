...
export const processChainData = async (): Promise<{
  ...
  const branchName = `add-orbit-chain/${stripWhitespace(validatedIncomingData.name)}`;
...
export const handleImages = async (
  ...
  console.log(`Fetching and saving chain logo: ${validatedIncomingData.chainLogo}`);
  ...
  if (validatedIncomingData.nativeTokenAddress && validatedIncomingData.nativeTokenLogo) {
    console.log(`Fetching and saving native token logo: ${validatedIncomingData.nativeTokenLogo}`);
    nativeTokenLogoPath =
...
export const updateOrbitChainsList = async (orbitChain: OrbitChain, targetJsonPath: string) => {
  ...
  const updatedOrbitChainsList = updateOrbitChainsFile(orbitChain, targetJsonPath);
...
export const extractImageUrlFromMarkdown = (markdown: string): string | null => {
  ...
  const htmlMatch = markdown.match(/<img[^>]+src\s*=\s*["']([^"']+)["'][^>]*\/?>/);
...
export const extractRawChainData = (issue: Issue): Record<string, string | boolean | undefined> => {
  const pattern = /###\s*(.*?)\s*\n\s*([\s\S]*?)(?=###|$)/g;
...
export const stripWhitespace = (text: string): string => text.replace(/\s+/g, '');
export const nameToSlug = (name: string): string => name.toLowerCase().replace(/\s+/g, '-');
...
export const fetchAndProcessImage = async (
  ...
  if (urlOrPath.startsWith('http')) {
    ...
    if (!fs.existsSync(localPath)) {
      throw new Error(`Provided local path '${localPath}' did not match any existing images.`);
    }
  ...
  if (!SUPPORTED_IMAGE_EXTENSIONS.includes(fileExtension.replace('.', ''))) {
    console.warn(`Unsupported image extension '${fileExtension}'. Converting to WEBP.`);
...
export const fetchAndSaveImage = async (urlOrPath: string, fileName: string): Promise<string> => {
  const { buffer, fileExtension } = await fetchAndProcessImage(urlOrPath);
  const imageSavePath = `images/${fileName}${fileExtension}`;

  const fullPath = path.join(process.cwd(), '../../packages/arb-token-bridge-ui/public/images');
...
export const transformIncomingDataToOrbitChain = async (
  ...
  const rollupData = await getArbitrumNetworkInformationFromRollup(chainData.rollup, provider);
...
export const updateOrbitChainsFile = (
  ...
  // Parse the JSON
  const orbitChains: { mainnet: OrbitChain[]; testnet: OrbitChain[] } = JSON.parse(fileContents);
  const networkType = orbitChain.isTestnet ? 'testnet' : 'mainnet';
